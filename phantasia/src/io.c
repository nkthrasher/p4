...
Do_chat(struct client_t *c, char *message, int announcement)
{
    int i;
    int count;
    char string_buffer[SZ_CHAT + 3], string_buffer2[SZ_CHAT + 3];
    struct event_t *event_ptr;

	/* if this is not an announcement, check for spam */
    if (!announcement) {
        ...
        count = 0;
        for (i = 0; i <= 9; i++) {
            if (c->chatLength[i] > 80) {
                count += floor(c->chatLength[i] / 80);
            }
        }
...
sprintf(string_buffer, "Count = %d \n", count);
Do_log(HACK_LOG, string_buffer);

            for (i = 0; i <= 9; i++) {

sprintf(string_buffer, "Message length %d = %d \n", i, c->chatLength[i]);
Do_log(HACK_LOG, string_buffer);
	...
	Do_replace_repetition(message);
            ...
            /* check for profanity */
        if (c->channel == 1 || c->broadcast) {
            if (Do_replace_profanity(message)) {

                Do_sin(c, .25 * c->swearCount);
		if (c->swearCount == 0) {

		    Do_dialog(c, "Please do not swear on channel 1.  Hit \"Info\" then \"Channel\" to switch.\n");

		   ++c->swearCount;
		}
		else if (c->swearCount == 1) {

		   Do_dialog(c, "Profanity is not allowed on channel 1.  Either leave the channel or stop swearing.\n");

		   ++c->swearCount;
		}
		else {
		    Do_caught_spam(c, H_PROFANITY);
		}
	    }
        }

            /* create a chat event */
        strcpy(string_buffer, c->modifiedName);
	if (c->broadcast) 
            strcat(string_buffer, " proclaims");
        strcat(string_buffer, ": ");
        strcat(string_buffer, message);
        strcat(string_buffer, "\n");
        sprintf(string_buffer2, "(%d) %s", c->channel, string_buffer);
    }

	/* if we're broadcasting, just copy the message over */
    else {
        strcpy(string_buffer, message);
        sprintf(string_buffer2, "(B) %s", message);
    }
    ...
    event_ptr->arg1 = (double) announcement;
    event_ptr->arg2 = 0;
    event_ptr->arg3 = strlen(string_buffer) + 1;
    event_ptr->arg4 = (void *) Do_malloc(event_ptr->arg3);
    strcpy(event_ptr->arg4, string_buffer);
    ...
    if (announcement) {
        ...
        Do_send_chat(c, event_ptr);
    }
	
        /* log the chat message */
    Do_log(CHAT_LOG, &string_buffer2);
}
