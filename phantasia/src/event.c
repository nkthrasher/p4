...
Do_send_chat(struct client_t *c, struct event_t *the_event)
{
    struct game_t *game_ptr;
    struct event_t *event_ptr;
	...
	if (game_ptr != c->game &&
		    ...
		    /* and we're on the channel this game is on */
		((game_ptr->description != NULL &&
		game_ptr->description->channel == c->channel) ||

		    /* or we are not in the game and the message is on 1 */
		(c->channel == 1 && game_ptr->description == NULL))) {
	    ...
	    /* if we're not pointing to ourselves */
	else if (game_ptr != c->game &&

		    /* and this player hears every channel */
		(game_ptr->hearAllChannels == HEAR_ALL ||

                    /* or this player hears channel 1 and it's on ch 1 */
		((game_ptr->hearAllChannels == HEAR_ONE) && 
                 (c->channel == 1)) ||

		    /* or if the player is on our coordinates */
		(game_ptr->description != NULL && !game_ptr->virtual &&
                game_ptr->x == c->player.x && game_ptr->y ==
                c->player.y) ||

		    /* or we are broadcasting */
		...
		/* modify argument 2 to reflect the channel it's on */
	    event_ptr->arg2 = (double) c->channel;
...
Do_file_event(struct client_t *c, struct event_t *the_event)
{
	...
	/* warning boxes go to the player */
    case DIALOG_EVENT:

        Do_dialog(c, the_event->arg4);
...
Do_handle_event(struct client_t *c, struct event_t *the_event)
{
	...
	if (c->player.gender == MALE) 
        ...
        Do_send_line(c, "Your sex has just been changed!\n");
