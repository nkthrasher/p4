...
Do_move(struct client_t *c, struct event_t *the_event)
{
	...
	Do_move_close(&the_event->arg1, &the_event->arg2, Anglemove(c));
        break;
...
*/
	...
	if (c->game != c->realm->king) {
		...
		pause = TRUE;
		Do_move_close(&the_event->arg1, &the_event->arg2, Maxmove(c));
	    }
...
Do_guru(struct client_t *c)
{
	...
	Do_poison(c, (double) -c->player.poison);
        ...
        Do_send_line(c, "He gives you a charm in recognition of your saintly life!\n");
...
Do_volcano(struct client_t *c)
{
    ...
    if (((c->player.type == C_HALFLING) && (c->player.sin <= ftemp)) ||
        ((c->player.type == C_DWARF)    && (c->player.sin * 2 <= ftemp)) ||
        ((c->player.type == C_ELF)      && (c->player.sin * 3 <= ftemp)) ||
        (c->player.sin * 4 <= ftemp)
       )
         {

        Do_send_line(c, "You throw the ring into the pool and it melts!\n");
        Do_more(c);
...
Do_information(struct client_t *c)
{
    ...
    /*
    strcpy(buttons.button[6], "Old Scores\n");
    */
...
int Maxmove(struct client_t *c) {
    if ((c->player.circle > 19) && (c->player.circle < 36)) {
        return MIN(ceil(c->player.level / 50.0) + 1.5, 10);
    } else {
        return MIN(floor((c->player.level * 1.5) + 1.5), 100);
    }
}

int Anglemove(struct client_t *c) {
    int temp;

    temp = floor(Maxmove(c) * .707106781);
