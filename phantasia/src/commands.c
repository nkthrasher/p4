...
Do_death(struct client_t *c, struct event_t *the_event)
{
	...
	case K_SQUISH:
            ...
            Do_send_line(c, "A giant hand appears in the heavens and descends.  Directly overhead, the hand clenches to a fist and swings toward you.\n");

	    sprintf(sb.how_died,
		    "was squished underneath the divine fist of %s",
		    the_event->arg4);
...
long answer;              /* pointer to option description */
    ...
    strcpy(theButtons.button[5], "Squish\n");
...
long answer;              /* pointer to option description */
    ...
    Do_clear_buttons(&theButtons, 3);
    strcpy(theButtons.button[3], "Warn\n");
    ...
    case 2:
        ...
        /* warn player */
    case 3:
        sprintf(string_buffer, "Who do you wish to warn?\n");
        if (Do_player_dialog(c, string_buffer, victim) != S_NORM) {
            return;
        }

        sprintf(string_buffer, "What is the text of the warning you wish to give? (%d chars max, \"default\" for default)\n", SZ_LINE);

        if (Do_string_dialog(c, warning, SZ_LINE, string_buffer)) {
            return;
        }

        if (!strcmp(warning, "default")) {
            strcpy(warning, "Your behavior is inappropriate.  Please stop or you will be disciplined.\n");
        } else {
            strcat(warning, "\n");
        }

        event_ptr->type = DIALOG_EVENT;
        event_ptr->arg4 = (void *) Do_malloc(SZ_LINE);
        strcpy(event_ptr->arg4, warning);

        if (!Do_send_character_event(c, event_ptr, victim)) {
            free((void *) event_ptr);
            Do_send_line(c, "That character just left the game.\n");
            Do_more(c);
            Do_send_clear(c);
        ...
        sprintf(string_buffer, 
            "[%s] %s warned %s the following : \"%s\"\n",
            c->connection_id, c->modifiedName, victim, warning);

/*
        sprintf(string_buffer, 
            "[%s] %s gave a warning to %s.\n",
            c->connection_id, c->modifiedName, victim);
*/

        Do_log(CANTRIP_LOG, string_buffer);

        Do_send_line(c, "The warning has been sent.\n");

        sprintf(string_buffer,
            "%s was given a warning for inappropriate behavior.\n",
            victim);
        Do_chat(c, string_buffer, TRUE);
    ...
    else if (event_ptr->type != DIALOG_EVENT) {
	Do_handle_event(c, event_ptr);
    } 
...
long answer;              /* pointer to option description */
    ...
    strcpy(theButtons.button[3], "Realm Clear\n");
	...
	c->realm->king_name[0] = '\0';
        ...
        Do_send_line(c, "Realm variables cleared.\n");
        return;
